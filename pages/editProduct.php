...
<body>
    ...
    <form action="api/handleProduct.php" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="id" <?php echo(isset($idProduct) ? "value=".$idProduct: "") ?> />

        <label for="productName">Nome prodotto*</label><br>
        <input type="text" id="productName" name="productName" required
            <?php echo(isset($product) ? "value=".htmlspecialchars($product['nome']) : "")?> 
        />
        <hr>

        <input type="checkbox" name="visible" id="visible" 
            <?php echo(isset($product['visibile']) ? 'checked' : '')?>/>
        ...
        <input type="file" name="3dPreview" id="3dPreview" required/>
        <hr>

        <label>Immagini esistenti:</label><br>
        <div id="immaginiEsistenti">
            <?php 
                if(isset($result_immagini)){ 
                    while ($img = mysqli_fetch_assoc($result_immagini)){ 
            ?>
